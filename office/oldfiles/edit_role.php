 <?php require_once('config/instantiated_files.php');
      ...
      if (is_dir($log_directory))
        ...
        $role_details = get_one_row_from_one_table_by_id('role_privileges','role_id',$rid,'date_added');
        $role_name = $role_details['role_name'];
        $pages_access = $role_details['pages_access'];
        $pages_access_dec = json_decode($pages_access);
    ...
    <div id="content-wrapper" class="d-flex flex-column">
      ...
      <div id="content">
        ...
        <div class="row">
              ...
              <form method="post" id="edit_role_privilege_form" enctype="multipart/form-data" class="user" action="">
                <label>Name of Role: <strong><?php echo $role_name; ?></strong> </label>    
                <input type="hidden" value="<?php echo $rid; ?>" id="role_id" name="role_id">                            
                
                <div class="form-group">
                    <label>Edit Pages this role can access: </label>   
                            ...
                            // if(  count(explode('.', $value)) <= 1   ){
                             $phpcheck = explode('.', $value);
                                 if(  $value != 'logout.php' && (count($phpcheck) == 2)  && $phpcheck[count($phpcheck) - 1] == 'php' && !in_array($value, $files_exception_arr) ){ 
                                  ...
                                  $pagge = explode('.', $value)[0];
                           ?>     
                        <tr>

                          <td>
                            <?php if( in_array($pagge, $pages_access_dec) ){?>
                            <input  checked  type="checkbox"  id="<?php echo explode('.', $value)[0]; ?>" name="<?php echo explode('.', $value)[0]; ?>">
                          <?php } else{?>
          ...
          <input type="submit" value="Edit User Role and Privileges" id="cmd_edit_user_role"  name="cmd_edit_user_role" class="btn btn-primary btn-sm btn-block"/>
                </a>
